...
def test_bundle_canonicalizes_roles_and_round_trips() -> None:
    k64 = _receipt(commit="7a4eec7dbb90627dde800bc1c3c90dbdd890d6d1", payload=b'{"schema":"flat.k6.4"}')
    trace = _receipt(commit="ca164387143cd58b5349d1e52850a2907071ed4c", payload=b'{"schema":"flat.m13b.4"}')
    ...
    assert [entry.role for entry in bundle.entries] == ["k6.4-envelope", "m13b4-trace"]
...
def test_entry_verifies_exact_receipt_identity() -> None:
    receipt = _receipt(commit="7a4eec7dbb90627dde800bc1c3c90dbdd890d6d1", payload=b'{"value":1}')
    entry = BikvEvidenceBundleEntryV1.from_receipt(role="k6.4-envelope", receipt=receipt)
...
def test_bundle_rejects_duplicate_roles() -> None:
    first = _receipt(commit="7a4eec7dbb90627dde800bc1c3c90dbdd890d6d1", payload=b'{"a":1}')
    second = _receipt(commit="ca164387143cd58b5349d1e52850a2907071ed4c", payload=b'{"b":2}')
...
def test_bundle_rejects_same_receipt_under_multiple_roles() -> None:
    receipt = _receipt(commit="7a4eec7dbb90627dde800bc1c3c90dbdd890d6d1", payload=b'{"a":1}')

    with pytest.raises(ValueError, match="multiple bundle roles"):
        BikvEvidenceBundleV1.from_receipts(
            [("candidate", receipt), ("dense-control", receipt)]
        )


def test_bundle_rejects_noncanonical_json_key_order() -> None:
    receipt = _receipt(commit="7a4eec7dbb90627dde800bc1c3c90dbdd890d6d1", payload=b'{"a":1}')
...
def test_bundle_rejects_reserved_or_invalid_roles() -> None:
    receipt = _receipt(commit="7a4eec7dbb90627dde800bc1c3c90dbdd890d6d1", payload=b'{"a":1}')
...
def test_bundle_is_provenance_only_and_does_not_require_scientific_fields() -> None:
    opaque = _receipt(
        commit="7a4eec7dbb90627dde800bc1c3c90dbdd890d6d1",
    ...
    assert bundle.entries[0].source_bytes == len(b'{"opaque":true}')
